...
void SPI2_Init(void)	
{
  ...
  SPI_InitTypeDef  SPI_InitStructure;
    ...
    GPIO_ResetBits(GPIOE,GPIO_Pin_1);			             //PF1输出低
...
char Rc522_Write(unsigned char address, unsigned char *date, unsigned char len)
{
	...
	unsigned char s=0x08;
			...
			if(status==MI_OK)//選卡成功
			{
			 ...
			 {
				status=MI_ERR;
			  status=PcdRead(s,date);

			  status=PcdWrite(s,date);
			  }
...
char Rc522_Read(unsigned char address, unsigned char *date, unsigned char len)
{
...
char Read_Card_ID(void)
{
	...
	char rc522_flag = 0;
	char rc522_stust = 0; //卡片状态
	
		rc522_stust = Rc522_Read(10,RFID_ID,10);
	
		if(rc522_stust != 0xff) //读取卡片成功
		{
			if(rc522_flag == 0) //卡片存在，读取成功
			{				
				rc522_flag = 1;
